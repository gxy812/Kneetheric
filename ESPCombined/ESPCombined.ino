...
class AngleStore {
    float angle[2] = {}; //remember to change max i
    float i = 0;
public:
    float getAngle() {
        float dist;
...
void loop() {
    ...
    if (check_av) {// read angular velocity every other loop
        current_v = ang.getAngularVelocity();
        check_av = !check_av;
    }
    // check for correct angle change and velocity
    if (isBleConnected & last_exec > 50) {
        if (current_angle < 90 && current_v < 5){
